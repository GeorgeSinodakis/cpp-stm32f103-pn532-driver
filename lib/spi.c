...
void SPI2_write(u8 *data, u8 dataLength)
{
	SPI2_assert();
	for(u8 i = 0; i < dataLength; i++)
	{
		SPI2_tranceiver(data[i]);
	}
	SPI2_deassert();
}

void SPI2_read(u8 *data, u8 dataLength)
{
	SPI2_assert();
	for(u8 i = 0; i < dataLength; i++)
	{
		data[i] = SPI2_tranceiver(0xff);
	}
	SPI2_deassert();
}

void SPI2_write_then_read(uint8_t *write_buffer, uint8_t write_len, uint8_t *read_buffer, uint8_t read_len)
{
	SPI2_assert();
	for(u8 i = 0; i < write_len; i++)
	{
		SPI2_tranceiver(write_buffer[i]);
	}
	for(u8 i = 0; i < read_len; i++)
	{
		read_buffer[i] = SPI2_tranceiver(0xff);
	}
	SPI2_deassert();
}
